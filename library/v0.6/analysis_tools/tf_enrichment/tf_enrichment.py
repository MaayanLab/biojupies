...
def results_table(enrichment_dataframe, source_label, target_label):
	...
	for gene_set_library in enrichment_dataframe['gene_set_library'].unique():
		...
		# Display gene set
		display(Markdown(gene_set_library.replace('ChEA_2016', '### A. ChEA (experimentally validated targets)').replace('ENCODE_TF_ChIP-seq_2015', '### B. ENCODE (experimentally validated targets)').replace('ARCHS4_TFs_Coexp', '### C. ARCHS4 (coexpressed genes)')))
...
def run(enrichr_results, signature_label):
	...
	for geneset in ['upregulated', 'downregulated']:
		...
		# Append ChEA results
		enrichment_dataframe = get_enrichr_results(enrichr_results[geneset]['userListId'], gene_set_libraries=['ChEA_2016', 'ENCODE_TF_ChIP-seq_2015', 'ARCHS4_TFs_Coexp'])
