...
def add_methods(notebook, notebook_configuration, normalization_methods, tool_metadata):

	# Initialize Methods
	methods = []

	# Add Data Methods
	methods.append('### Data Processing')

	# Add Signature Methods
	if len(notebook_configuration['signature']):
		methods.append('### Signature Generation')

	# Add Normalization Methods
	if len(normalization_methods):
		methods.append('### Data Normalization')

	# Add Tool Methods
	for tool_configuration in notebook_configuration['tools']:
		methods.append(tool_metadata[tool_configuration['tool_string']]['methods'].format(**tool_configuration['parameters']))
...
def add_references(notebook, notebook_configuration, tool_metadata):

	# Add References
	selected_tools = [x['tool_string'] for x in notebook_configuration['tools']]
	cell = "---\n## <span id='references'>References</span>\n"+'<br>'.join(['<a href="{reference_link}">{reference}</a><br>'.format(**tool_metadata[x]) for x in selected_tools])
	return addCell(notebook, cell, 'markdown')
...
def generate_notebook(notebook_configuration, tool_metadata):
	...
	# Add Intro
	notebook = add_introduction(notebook=notebook, notebook_configuration=notebook_configuration, tool_metadata=tool_metadata)
	...
	# Add Methods
	notebook = add_methods(notebook, notebook_configuration=notebook_configuration, normalization_methods=normalization_methods, tool_metadata=tool_metadata)

	# Add References
	notebook = add_references(notebook, notebook_configuration=notebook_configuration, tool_metadata=tool_metadata)
